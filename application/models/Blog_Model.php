...
//        if ($lang_id == "17") {
	...
	function getActiveBlog( $pg, $cat, $search )
    {
        $lang_id = ($this->session->userdata('language_id') ? $this->session->userdata('language_id') : 17);

        $this->db->select("mbp.*");
		if(!empty($search) ){ 
			$this->db->like('mbp.post_title',"$search");
		}
        $this->db->from(TABLES::$MST_BLOG_POSTS . ' as mbp');
        $this->db->join('tbl_mst_categories as mu', 'mu.category_id=mbp.blog_category');
		
		if(!empty($cat) && 'cat' == $pg ){ 
			$this->db->where('mbp.blog_category', $cat);
		}
        $this->db->where('mbp.status', '1');
        $this->db->order_by('mbp.posted_on', 'desc');
