...
class Express_checkout extends CI_Controller {
    ...
    function SetExpressCheckout() {
        ...
        $SECFields = array(
            ...
            'hdrimg' => 'http://sassconsultants.rebelute.in/media/frontend/assets/base/img/layout/logos/LOGO_SA.png', // URL for the image displayed as the header during checkout.  Max size of 750x90.  Should be stored on an https:// server or you'll get a warning message in the browser.
            'logoimg' => 'http://sassconsultants.rebelute.in/media/frontend/assets/base/img/layout/logos/LOGO_SA.png', // A URL to your logo image.  Formats:  .gif, .jpg, .png.  190x60.  PayPal places your logo image at the top of the cart review area.  This logo needs to be stored on a https:// server.
            'brandname' => 'Saas Consultant', // A label that overrides the business name in the PayPal account on the PayPal hosted checkout pages.  127 char max.
        ...
        $Payment = array(
			...
			'shiptostreet' =>  $cart['shiptostreet'], 					// Required if shipping is included.  First street address.  100 char max.
			'shiptostreet2' => $cart['shiptostreet'], 	 					// Second street address.  100 char max.
			'shiptocity' => $cart['shiptocity'], 					// Required if shipping is included.  Name of city.  40 char max.
			'shiptostate' => $cart['shiptostate'], 					// Required if shipping is included.  Name of state or province.  40 char max.
			'shiptozip' => $cart['shiptozip'], 						// Required if shipping is included.  Postal code of shipping address.  20 char max.
			'shiptophonenum' => $cart['shiptophonenum']  				// Phone number for shipping address.  20 char max.
        );
    ...
    function GetExpressCheckoutDetails() {
        ...
        if (!$this->paypal_pro->APICallSuccessful($PayPalResult['ACK'])) {
			...
			$cart['paypal_transaction_id'] = isset($PayPalResult['TRANSACTIONID']) ? $PayPalResult['TRANSACTIONID'] : '';
